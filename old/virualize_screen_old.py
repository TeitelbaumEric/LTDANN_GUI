        ...
        with serial.Serial(self.port_name, self.baud_rate) as ser:
            while True:
                try:
                    ...
                    node_dict[info[0]] = {'rssi': info[1], 'snr': info[1]}
...
class Screen3(QWidget):
    ...
    def display_predictions(self):
        ...
        if len(node_dict) > 3:
            ...
            if self.unknown_node['x'] != -9999 and self.unknown_node['y'] != -9999:
                self.matplotlib_canvas.axes.scatter(self.unknown_node['x'], self.unknown_node['y'], 500, s=100, color='blue', label='Unknown Object')

            # Generating points within the expected intersection region
            num_points = 100000
            points = np.random.rand(num_points, 3) * 800  # Generates points within [0, 1000] in 3D space
            ...
            while not points.size > 0:
                ...
                num_points = 100000
                points = np.random.rand(num_points, 3) * 800  # Generates points within [0, 1000] in 3D space
                points += np.random.rand(num_points, 3) * -800
...
# node_dict = {} # Stores node information received from the serial port
...
#             # Generating points within the expected intersection region
#             num_points = 100000
#             points = np.random.rand(num_points, 3) *  800  # Generates points within [0, 1000] in 3
#             for center, radius in zip(nodes, predicted_distances):
#                 points = self.sphere_intersection_points(center, radius, points)
